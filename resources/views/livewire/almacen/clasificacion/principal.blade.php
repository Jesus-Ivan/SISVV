<div class="ms-3 mx-3">
    <label for="countries" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">Agregar Nueva:</label>
    <!--CAMPO DE ENTRADA PARA NUEVA CLASIFICACION-->
    <div class="inline-flex">
        <form class="w-fit">
            <select id="clasificacion" wire:model = 'tipo'
        ...
        <div>
            <button type="button" wire:click = "register"
                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 ms-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Guardar
            </button>
        </div>
    ...
    <div class="relative w-96 my-2">
        ...
        <input wire:model.live.debounce.500ms="search" type="search" id="search"
            ...
            placeholder="Buscar nombre o código" required />
    </div>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg my-2">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($listaClasificacion as $clasificacion)
                    <tr wire:key={{ $clasificacion->id }}
                        class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        @if ($this->edit_id_clasificacion == $clasificacion->id)
                            <th scope="row"
                            ...
                            <td class="px-6 py-2 min-w-80">
                                ...
                                <div class="dark:text-white">
                                    <input type="text" id="nombre" wire:model='edit_nombre_clasificacion'
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                                    @error('edit_nombre_clasificacion')
                                        <x-input-error messages="{{ $message }}" />
                                    @enderror
                                </div>
                            </td>
                            <!-- TIPO -->
                            <td class="px-6 py-2">
                                <select id="tipo" wire:model='edit_tipo_clasificacion'
                                    ...
                                    <option value="FAMILIA">FAMILA</option>
                                    <option value="CATEGORÍA">CATEGORÍA</option>
                                </select>
                                @error('edit_tipo_clasificacion')
                                    <x-input-error messages="{{ $message }}" />
                                @enderror
                            </td>
                            <!-- ESTADO -->
                            <td class="px-6 py-2">
                                <select id="estado" wire:model='edit_estado_clasificacion'
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-fit p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    <option selected value="{{ null }}">Seleccione</option>
                                    <option value="1">ACTIVO</option>
                                    <option value="0">INACTIVO</option>
                                </select>
                                @error('edit_estado_clasificacion')
                                    <x-input-error messages="{{ $message }}" />
                                @enderror
                            </td>
                            <!-- BOTONES DE EDICION -->
                            <td class=" px-6 py-2 text-center">
                                <button type="button" wire:click='confirmarEdit({{ $clasificacion->id }})'
                        ...
                        @else
                            ...
                            <td class="px-6 py-1 text-center">
                                <button type="button" wire:click='editClasificacion({{ $clasificacion->id }})'
        ...
        <div class="w-full my-2 flex justify-end">{{ $listaClasificacion->links() }}</div>
    </div>

    @include('livewire.almacen.clasificacion.modal-alerta')
