<div>
    <div class="flex gap-2">
        {{-- Barra de busqueda --}}
        <div>
            <label for="modificadores" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">Descripción
                Grupo</label>
            <div class="inline-flex">
                <!-- INPUT -->
                <div class="relative w-96">
                    <input type="text" id="descripcion" wire:model='descripcionModif'
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg my-2" style="max-height: 300px; overflow-y: auto;">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($listaModificadores as $modificador)
                    <tr wire:key='{{ $modificador->id }}'
                        class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                        @if ($this->edit_id_modificador == $modificador->id)
                            <th scope="row"
                                class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $modificador->id }}
                            </th>
                            <td class="px-6 py-2">
                                <input type="text" id="editDescripcionInsum" wire:model='edit_descripcionModif'
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                                @error('edit_descripcionModif')
                                    <x-input-error messages="{{ $message }}" />
                                @enderror
                            </td>
                            <td class="px-6 py-2 text-center">
                                <div>
                                    <button type="button" wire:click='updateModificador({{ $modificador->id }})'
                        ...
                        @else
                            ...
                            <td class="px-6 py-1 text-center">
                                <div>
                                    <button type="button" wire:click='editModificador({{ $modificador->id }})'
                                    ...
                                    <button type="button" wire:click='deleteModificador({{ $modificador->id }})'
