<div class="m-2">
    {{-- FILTRO DE BUSQUEDA --}}
    <form class="w-full flex gap-3" wire:submit='search' method="GET" x-data='{ selected_grupo: $wire.grupo }'>
        @csrf
        {{-- Search input --}}
        ...
        <select id="Estado" wire:model='estado'
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            <option selected value="{{ null }}">SELECCIONE ESTADO</option>
    ...
    <div class="my-2">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($this->productos as $index => $articulo)
                        <tr wire:key='{{ $articulo->clave }}'
                            ...
                            <td class="px-6 py-1 uppercase">
                                {{ $articulo->subgrupo->descripcion }}
                            </td>
                            <td class="px-6 py-1 uppercase">
                                $ {{ $articulo->precio_con_impuestos }}
                            </td>
                            <td class="px-6 py-1">
                            ...
                            <td class="px-3 py-1">
                                <div class="text-center">
                                    <a href="{{ route('almacen.productos.editar', $articulo->clave) }}">
                                        <button type="button"
