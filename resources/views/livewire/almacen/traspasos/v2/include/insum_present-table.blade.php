...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg my-2 h-80">
        ...
        <tbody>
            @foreach ($lista_articulos as $index => $item)
                <tr
                    ...
                    <td class="px-6 py-2">
                        <button type="button" wire:click='detallesPresentacion({{ $index }})'
                            ...
                            {{ number_format($item['cantidad_insumo'] ?? 0, 3) }}
                            ...
                            <!--Loading indicator-->
                            <div class="flex justify-center ms-2" wire:loading.delay
                                wire:target='detallesPresentacion'>
    ...
    <x-slot name='body'>
        ...
        <div class="h-auto max-w-4xl overflow-y-auto">
            ...
            <div class="p-1 w-full max-w-2xl max-h-full">
                <!-- Result table-->
                <div class="relative overflow-x-auto h-72 shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th scope="col" class="px-6 py-3">
                        ...
                        <tbody>
                            @foreach ($insumXpresent as $index => $presentacion)
                                <tr wire:key='{{ $index }}'
                                    class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                                    <th scope="row"
                                    ...
                                    <td class="px-6 py-2">
                                        <input type="number"
                                            wire:model.live='insumXpresent.{{ $index }}.cantidad'
                                            wire:change='actualizarCantidadModal({{ $index }})'
                                    ...
                                    <td class="px-6 py-2">
                                        {{ number_format($presentacion['cantidad_insumo'], 3) }}
                                    </td>
                                </tr>
                            @endforeach
            ...
            <div class="flex items-center mt-3 border-gray-200 rounded-b dark:border-gray-600">
                <div class="flex grow">
                    <button type="button" wire:click='guardarPresentaciones'
                        class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 my-2 inline-flex items-center text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Aceptar
                        <!--Loading indicator-->
                        <div class="flex justify-center ms-2" wire:loading.delay wire:target='guardarPresentaciones'>
                            @include('livewire.utils.loading', ['w' => 4, 'h' => 4])
                        </div>
                    </button>
                </div>
                <p>CANTIDAD INSUMO: {{ number_format($totalInsumoModal, 3) }}</p>
            </div>
        </div>
    </x-slot>
</x-modal>
