<div class="relative overflow-x-auto shadow-md sm:rounded-lg ms-3 mx-3 my-3">
        ...
        <tbody>
            @foreach ($this->ventasPendientes as $venta)
                <tr wire:key ='{{ $venta->folio_venta }}'
                    ...
                    <td class="px-6 py-2">
                        <a href="{{ route('pv.ventas.pagar', ['codigopv' => $codigopv, 'folioventa' => $venta->folio_venta]) }}" target="_blank"
                        ...
                        <a href="{{ route('ventas.ticket', ['venta' => $venta->folio_venta]) }}" target="_blank"
    ...
    <div>{{ $this->ventasPendientes->links() }}</div>
</div>
