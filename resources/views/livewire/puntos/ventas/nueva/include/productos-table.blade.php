            ...
            <tbody>
                @foreach ($this->ventaForm->productosTable as $productoIndex => $producto)
                    <tr wire:key="{{ $productoIndex }}"
                        ...
                        {{-- DESCRIPCION --}}
                        <td class="w-96 max-w-96 px-3 py-2  text-gray-900 whitespace-nowrap dark:text-white">
                            <p class="font-bold">
                                {{ $producto['nombre'] ?: $producto['catalogo_productos']['nombre'] }}</p>
                            <input type="text"
                        ...
                        <td class="px-2 py-2">
                            @if (!array_key_exists('modif', $producto) && !array_key_exists('id', $producto))
